resource "local_file" "inventory" {
  content = <<-DOC
    ...
    nginx:
      hosts:
        nginx-node:
          ...
          ansible_host: ${yandex_compute_instance.nginx-node.network_interface.0.ip_address}
    mysql:
      hosts:
        mysql-master:
          ansible_ssh_common_args: '-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ProxyCommand="ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -W %h:%p -q ${var.yandex_ubuntu_user}@${var.yandex_external_ip}"'
    ...
    gitlab:
      hosts:
        gitlab-server:
          ansible_ssh_common_args: '-o ControlPersist=60s -o ServerAliveInterval=30 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ProxyCommand="ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -W %h:%p -q ${var.yandex_ubuntu_user}@${var.yandex_external_ip}"'
          ansible_user: ${var.yandex_centos_user}
